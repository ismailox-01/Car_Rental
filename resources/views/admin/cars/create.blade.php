...
@section('page_title', isset($car) ? 'Edit Car: ' . $car->brand . ' ' . $car->model : 'Add New Vehicle')
            ...
            <form action="{{ isset($car) ? route('admin.cars.update', $car) : route('admin.cars.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4 mb-5">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" name="year" class="form-control bg-light border-0 py-2 shadow-none" required value="{{ old('year', $car->year ?? date('Y')) }}" min="2000" max="{{ date('Y') + 1 }}">
                    </div>
                    <div class="col-md-2">
                ...
                <div class="row g-4 mb-5">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" name="seats" class="form-control bg-light border-0 py-2 shadow-none" required value="{{ old('seats', $car->seats ?? 5) }}" min="1">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold small text-uppercase">Luggage (Bags)</label>
                        <input type="number" name="luggage" class="form-control bg-light border-0 py-2 shadow-none" required value="{{ old('luggage', $car->luggage ?? 2) }}" min="0">
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <div class="form-check form-switch mt-4 pt-2">
                            <input class="form-check-input" type="checkbox" name="air_conditioning" value="1" id="acSwitch" {{ old('air_conditioning', $car->air_conditioning ?? 1) ? 'checked' : '' }}>
                ...
                <div class="row g-4 mb-5">
                    ...
                    <div class="col-md-6">
                        <div class="form-check form-switch bg-light p-3 rounded-4 ps-5">
                            <input class="form-check-input ms-0 me-3" type="checkbox" name="is_available" value="1" id="availSwitch" {{ old('is_available', $car->is_available ?? 1) ? 'checked' : '' }}>
                            <label class="form-check-label fw-bold" for="availSwitch">Show as Available for Rent</label>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-check form-switch bg-light p-3 rounded-4 ps-5">
                            <input class="form-check-input ms-0 me-3" type="checkbox" name="is_featured" value="1" id="featSwitch" {{ old('is_featured', $car->is_featured ?? 0) ? 'checked' : '' }}>
                ...
                <div class="d-flex justify-content-end gap-3 pt-4 border-top">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-5 fw-bold shadow-sm">
                        {{ isset($car) ? 'SAVE CHANGES' : 'CREATE VEHICLE' }}
                    </button>
                </div>
            </form>
        </div>
